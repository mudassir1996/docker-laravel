                    ...
                    <div class="card">
                        ...
                        @endphp --}}
                            <form class="d-inline" method="POST"
                                action="{{ route('employee_verification.resend') }}">
                                @csrf
                                <button type="submit"
                                    class="btn btn-link p-0 m-0 align-baseline">{{ __('click here to send link.') }}</button>.
                            </form>
                            <form id="logout-form" action="{{ route('employee.logout') }}" method="POST">
                                @csrf
                                <input type="submit" value="Logout" class="mt-5 btn btn-success font-size-h5 px-12">
                            </form>
                        </div>
                    </div>
                </div>
