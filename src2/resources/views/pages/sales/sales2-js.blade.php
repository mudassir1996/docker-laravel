<script>
    ...
    function addToCart(product) {
        ...
        $.ajax({
            url: "{{ url('products-in-cart') }}?query=" + product.id,
            type: "GET",
            dataType: "json",
            success: function(res) {
                ...
                if ($('#hidden_row_id_' + id_number).val() != undefined) {
                    ...
                    output += '<input type="hidden" id="hidden_row_id_' + id_number + '" value="' +
                        id_number + '">'
                    ...
                    output += '</div>';
                    output += '<a href="#" class="text-dark text-hover-primary">' + product.product_title +
                        '</a>';
                    output += '<input type="hidden" name="order_id">';
                    output += '<input type="hidden" name="product_id[]" value="' + product.product_id +
                        '">';
                    output += '<input type="hidden" id="units_' + count + '" value="' + product
                        .units_in_stock + '">';
                    output += '<input type="hidden" id="stock_keeping_' + count + '" value="' + product
                        .stock_keeping + '">';
                    output += '<input type="hidden" id="allow_half_' + count + '" value="' + product
                        .product_allow_half + '">';
                    output += '</td>';
                    output +=
                        '<td class="text-right align-middle font-weight-bolder font-size-h5 price" id="price_' +
                        count + '">' + product.retail_price + '</td>';
                    output += '<input type="hidden" name="cost_price[]" id="cost_price_' + count +
                        '" value="' + product.cost_price + '">';
                    output += '<input type="hidden" name="retail_price[]" id="retail_price_' + count +
    ...
    //     $.ajax({
    //         url: "{{ url('products-in-cart') }}?query=" + product_id,
    //         type: "GET",
    //         dataType: "json",
    //         success: function(res) {
    ...
    // $('#search-barcode').keyup(function(e) {
    //     if (e.keyCode == 13) {
    //         var barcode = $(this).val();
    //         if (barcode != '') {
    //             $.ajax({
    //                 url: "{{ url('live-search') }}?barcode=" + barcode,
    //                 type: "Get",
    //                 success: function(res) {
    //                     data = res['search_result'];
    //                     $('#product_search_table > tbody').html(res['output']);
    ...
    function select_product(key) {
        ...
        if ($('#hidden_row_id_' + id_number).val() != undefined) {
            ...
            output += '<td class="d-flex align-items-center  font-weight-bolder">';
            output += '<div class="symbol symbol-50 flex-shrink-0 mr-4 bg-light">';
            output += '<div class="symbol-label" style="background-image: url(' + "'" + imagefolder + '/' + product
                .product_feature_img + "'" + ')"></div>';
            output += '</div>';
            ...
            output += '</td>';
            output += '<td class="text-right align-middle font-weight-bolder font-size-h5 price" id="price_' + count +
                '">' + product.retail_price + '</td>';
            output += '<input type="hidden" name="cost_price[]" id="cost_price_' + count + '" value="' + product
                .cost_price + '">';
            output += '<input type="hidden" name="retail_price[]" id="retail_price_' + count + '" value="' + product
                .retail_price + '">';
            ...
            output += '<input type="number" id="discount_per_' + count +
                '" name="discount_percentage[]" value="0" class="border border-success rounded bg-gray-100 px-1 py-1 col-xl-7 discount_per">';
            output += '</td>';
            output += '<td class="text-right align-middle font-weight-bolder font-size-h5 total" id="total_' + count +
                '"></td>';
            output +=
                '<input type="hidden"  name="tax_value[]" value="0" class="border border-success rounded bg-gray-100 px-1 py-1 col-xl-6 discount_per">';
            output +=
    ...
    $('#payment').on("click", function() {
        // console.log($('#customer option:selected').attr('data-customer'));
        if ($("#product_table > tbody > tr").html() == undefined) {
            toastr.error("Cannot submit empty form");
        } else if ($('#customer').val() == '') {
            toastr.error("Please select customer");
        } else if ($('#customer option:selected').attr('data-customer') == 0 && ($('#payment_type_dropdown')
                .val() == 'credit' || $('#payment_type_dropdown').val() == 'split_bill')) {
        ...
        } else if ($('#payment_type_dropdown').val() == 'split_bill' && $('#amount_paid').val() ==
            amount_payable) {
    ...
    $('#btn').on('click', function(event) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $.ajax({
                    url: "{{ url('get-customer') }}?id=" + response,
                    type: "Get",
                    success: function(res) {
                        $("#customer").append("<option data-customer=" + res
                            .allow_credit + " value='" + res.id + "'>" + res
...
<script type="text/javascript">
    $('#view-hold-order').on('click', function(event) {
        ...
        $.ajax({
            url: "{{ url('get-hold-orders') }}?customer_id=" + customer_id,
            type: "Get",
            success: function(res) {
    ...
    function select_hold_order(key) {
        ...
        hold_order = data[key];
        // console.log(hold_orders)
        // // console.log(hold_order.sales_order_detail);
        // console.log(key);
        $('#product_table > tbody').empty();
        // let count=0;
        ...
        $.each(hold_order.sales_order_detail,
            function(key, hold_order_detail) {
                ...
                // }, 2000);
                $.ajax({
                    url: "{{ url('get-product-data') }}?product_id=" + id_number,
                    type: "Get",
                    success: function(result) {
                        ...
                        output += '<td class="d-flex align-items-center  font-weight-bolder">';
                        output += '<div class="symbol symbol-50 flex-shrink-0 mr-4 bg-light">';
                        output += '<div class="symbol-label" style="background-image: url(' + "'" +
                            imagefolder + '/' + product_data.product_feature_img + "'" + ')"></div>';
                        output += '</div>';
                        output += '<a href="#" class="text-dark text-hover-primary">' + product_data
                            .product_title + '</a>';

                        output += '<input type="hidden" name="product_id[]" value="' + hold_order_detail
                            .product_id + '">';
                        output += '<input type="hidden" id="units_' + count + '" value="' + product_data
                            .units_in_stock + '">';
                        output += '<input type="hidden" id="stock_keeping_' + count + '" value="' +
                            product_data.stock_keeping + '">';
                        output += '<input type="hidden" id="allow_half_' + count + '" value="' +
                            product_data.product_allow_half + '">';
                        output += '</td>';
                        output +=
                            '<td class="text-right align-middle font-weight-bolder font-size-h5 price" id="price_' +
                            count + '">' + hold_order_detail.retail_price + '</td>';
                        output += '<input type="hidden" name="cost_price[]" id="cost_price_' + count +
                            '" value="' + product_data.cost_price + '">';
                        output += '<input type="hidden" name="retail_price[]" id="retail_price_' +
                            count + '" value="' + product_data.retail_price + '">';
                        output +=
                            '<input type="hidden" name="total_cost[]" class="total_cost" id="total_cost_' +
                            count + '">';
                        output +=
                        ...
                        output += '<td class="text-center align-middle">';
                        output += '<input type="number" name="quantity[]" id="quantity_' + count +
                            '" value="' + hold_order_detail.quantity +
                        ...
                        output +=
                            '<td class="text-right align-middle font-weight-bolder font-size-h5 total" id="total_' +
                            count + '">' + hold_order_detail.amount_payable + '</td>';
                        output +=
                            '<input type="hidden"  name="tax_value[]" value="0" class="border border-success rounded bg-gray-100 px-1 py-1 col-xl-6 discount_per">';
                        output +=
                            '<input type="hidden" name="tax_percentage[]" value="0" class="border border-success rounded bg-gray-100 px-1 py-1 col-xl-6 discount_per">';
