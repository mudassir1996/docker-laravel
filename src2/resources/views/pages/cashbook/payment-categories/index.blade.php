                    ...
                    <div class="card card-custom">
                        <div class="card-header flex-wrap py-5">
                            <div class="card-title">
                                <h3 class="card-label">Payment Category
                                    <span class="d-block text-muted pt-2 font-size-sm">All Payment Categories</span>
                                </h3>
                            </div>
                            <div class="card-toolbar">
                                ...
                                @if (!Auth::guard('web')->check())
                                    @can('payment_category_create')
                                        <div class="btn-group">
                                            <a type="button" href="{{ route('payment-categories.create') }}"
                                            ...
                                            <div class="dropdown-menu">
                                                <a href="#" type="button" class="dropdown-item" data-toggle="modal"
                                                    data-target="#upload_model">Import categories</a>
                                                <a href="{{ route('standard-category') }}" class="dropdown-item">Import
                                                    standard categories</a>
                                            </div>
                                        </div>
                                    @endcan
                                @else
                        ...
                        <div class="card-body">
                            ...
                            <table class="table table-separate table-head-custom nowrap table-checkable"
                                id="kt_datatable_payment_category">
                                <thead>
                                    <tr>
                                        <th>ID</th>
                                        <th>Title</th>
                                    ...
                                    @if (!$payment_categories->isEmpty())
                                        @foreach ($payment_categories as $payment_category)
                                            <tr>
                                                ...
                                                <td>
                                                    {{ ucfirst($payment_category->payment_category_status) }}
                                                </td>
                                                <td>{{ substr($payment_category->payment_category_description, 0, 40) }}...
                                                </td>
                                                <td>
                                                    {{ $payment_category->employee_name }}
                                                </td>
                                                <td>{{ $payment_category->created_at }}</td>
                                                <td>{{ $payment_category->updated_at }}</td>
                                                <td>
                                                    <form
                                                        action="{{ route('payment-categories.destroy', $payment_category->id) }}"
                                                        id="delete_item_from{{ $payment_category->id }}" method="post">
                                                        @csrf
                                                        @method('DELETE')
                                                        @if (!Auth::guard('web')->check())
                                                            {{-- @can('payment_category_show')
                                                                <a class="btn p-0" title="View"
                                                                    href="{{ route('payment-categories.show', $payment_category->id) }}">
                                                            ...
                                                            @can('payment_category_edit')
                                                                <a class="btn p-0" title="Edit"
                                                                    href="{{ route('payment-categories.edit', $payment_category->id) }}">
                                                            ...
                                                            @can('payment_category_delete')
                                                                <a class="btn p-0" title="Delete"
                                                                    onclick="deleteConfirmation('delete_item_from{{ $payment_category->id }}')"><i
