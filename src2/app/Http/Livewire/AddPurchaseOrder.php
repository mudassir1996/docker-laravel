...
class AddPurchaseOrder extends Component
{
    ...
    {
        $this->suppliers = Supplier::where('outlet_id', session('outlet_id'))->latest()->select('id', 'supplier_title')->get();
        ...
        $this->orderProducts = collect($this->orderProducts)
            ->map(function ($orderProducts) use ($data) {
                ...
                if ($data == 'status' && $this->purchase_order['po_status'] != 'delivered') {
                    ...
                    $orderProducts['new_cost'] = $orderProducts['old_cost'];
    ...
    public function add_purchase_order_to_db()
    {
        ...
        DB::transaction(
            function () {
                ...
                $po = new InventoryPurchaseOrder($this->purchase_order);

                if ($this->purchase_order['po_status'] != 'delivered') {
                    $po->po_purchased_date = null;
                }
                ...
                if ($this->purchase_order['po_status'] == 'delivered' && $payment_type->value == '0') {
                    $transaction_request = new Request([
                        ...
                        'payment_date'  => $this->purchase_order['po_purchased_date'],
                        'system_remarks' => 'purchase_orders',
                        'description' => "Transaction added from purchase order",
                        'order_id' =>  $po->id,
                ...
                // if ($this->purchase_order['po_status'] == 'delivered' && $this->purchase_order['payment_type'] == '3') {
                //     $balance = SupplierAccount::where('supplier_id', $this->purchase_order['supplier_id'])->latest()->pluck('balance')->first();
                //     $balance = $balance ?? 0;
                //     $supplier_transaction = new SupplierAccount(
                //         [
                //             'amount'  => abs($this->purchase_order['amount_payable']),
                //             'balance'  => $balance - abs($this->purchase_order['amount_payable']),
                //             'payment_type'  => $this->purchase_order['payment_type'],
                //             'description'  => 'Split purchase order transaction created from Purchase Orders.',
                //             'payment_date'  => Carbon::now(),
                //             'payment_method_id'  => $this->purchase_order['payment_method_id'],
                //             'order_id' => $po->id,
                //             'supplier_id' =>  $this->purchase_order['supplier_id'],
    ...
    public function render()
    {
        ...
        return view('livewire.add-purchase-order');
    }
}
