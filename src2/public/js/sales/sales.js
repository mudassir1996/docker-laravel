    ...
    $('#payment').on("click", function() {
        ...
        else if ($('#customer option:selected').attr('data-customer') == 0 && ($('#payment_type_dropdown').find(':selected').attr('data-value')=='1' || $('#payment_type_dropdown').find(':selected').attr('data-value')=='2')) {
        ...
        else if($('#payment_type_dropdown').find(':selected').attr('data-value')=='2' && $('#amount_paid').val() <= 0){
        ...
        // }
         else if ($('#payment_type_dropdown').find(':selected').attr('data-value')=='0' && $('#amount_paid').val() < amount_payable) {
            ...
            function(key, return_order_detail) {
                ...
                $.ajax({
                    ...
                    success: function(result) {
                        ...
                        output = '<tr id="row_' + return_order_detail.product_id + '">';
                        output += '<input type="hidden" id="hidden_row_id_' + product_data.product_id + '" value="' + return_order_detail.product_id + '">'
                        output += '<td class="d-flex align-items-center  font-weight-bolder">';
                        output += '<div class="symbol symbol-50 flex-shrink-0 mr-4 bg-light">';
                        output += '<div class="symbol-label" style="background-image: url(' + "'" + imagefolder + '/' + product_data.product_feature_img + "'" + ')"></div>';
                        output += '</div>';
                        ...
                        output += '<input type="hidden" name="amount_payable[]" value="0" class="amount_payable" id="amount_payable_' + count + '">';
                        output += '<td class="text-left align-middle">';
                        output += '<a href="#" class="remove " id="' + return_order_detail.product_id + '" title="Remove">';
                        output += '<i class="fas fa-window-close text-danger text-focus-primary"></i>';
                        output += '</a>';
                        output += '</td>';
                        output += '</tr>';
