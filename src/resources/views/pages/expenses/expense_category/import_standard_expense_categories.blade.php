            ...
            <div class="d-flex flex-row">
                ...
                <div class="flex-row-fluid">
                    ...
                    <div class="card card-custom card-sticky" id="kt_page_sticky_card">
                        ...
                        <div class="card-body">
                            <!--begin::Form-->
                            <form method="post" action="{{ route('standard-expense-category.store') }}" id="customer_transaction_form">
                                @csrf
                                    ...
                                    <tbody>
                                        @foreach ($standard_expense_categories as $standard_expense_category)
                                            <tr>
                                                <td>
                                                    <label class="checkbox">
                                                        <input type="checkbox" class="check" value="{{$standard_expense_category->id}}" name="category_id[]"/>
                                                        <span></span>
                                                    </label>
                                                    
                                                </td>
                                                <td>
                                                    @if ($standard_expense_category->featured_img=='placeholder.jpg')
                                                        @php
                                                            $image=asset('storage/'.$standard_expense_category->featured_img);
                                                        @endphp
                                                    @elseif (Storage::disk('public')->exists('categories/'.basename($standard_expense_category->featured_img)))
                                                        @php
                                                            $image=asset('storage/categories/'.$standard_expense_category->featured_img);
                                                        @endphp
                                                    @else
                                                        @php
                                                            $image=$standard_expense_category->featured_img
                                                        @endphp
                                                    @endif
                                                    <div class="symbol symbol-75  flex-shrink-0">
                                                        <div class="symbol-label"> <img class="h-100 w-100 align-self-end" src="{{$image}}" alt="{{$standard_expense_category->title}}"> </div>
                                                    </div>
                                                </td>
                                                <td>
                                                    {{$standard_expense_category->title}}
                                                </td>
                                                <td>
                                                   {{$standard_expense_category->description}}
                                                </td>
                                            </tr>
                                        @endforeach
