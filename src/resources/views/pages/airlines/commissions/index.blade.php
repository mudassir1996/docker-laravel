...
@section('title', 'Outlet Commissions')
@section('content')
                ...
                <div class="d-flex align-items-baseline flex-wrap mr-5">
                    ...
                    <ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
                        <li class="breadcrumb-item">
                            <a href="#" class="text-muted">All Commissions</a>
                        </li>
                    </ul>
                    <!--end::Breadcrumb-->
                </div>
                    ...
                    <div class="card card-custom">
                        <div class="card-header flex-wrap py-5">
                            <div class="card-title">
                                <h3 class="card-label">Outlet Commission
                                    <span class="d-block text-muted pt-2 font-size-sm">All Commissions</span>
                                </h3>
                            </div>
                            <div class="card-toolbar">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-sm dropdown-menu-right">
                                        ...
                                        <ul class="navi flex-column navi-hover py-2">
                                            <li
                                                ...
                                                Choose an option:</li>
                                            <li class="navi-item">
                                                <a href="{{ route('print.categories') }}" target="_blank"
                                                    class="navi-link">
                                ...
                                @if (!Auth::guard('web')->check())
                                    @can('commission_create')
                                        <a type="button" href="{{ route('outlet-commissions.create') }}"
                        ...
                        <div class="card-body">
                            ...
                            <table class="table table-separate table-head-custom nowrap table-checkable"
                                ...
                                <tbody>
                                    @if (!$outlet_commissions->isEmpty())
                                        @foreach ($outlet_commissions as $outlet_commission)
                                            <tr>
                                                <td>{{ $outlet_commission->commission_title }}</td>
                                                <td>{{ $outlet_commission->commission_type }}</td>
                                                <td>{{ $outlet_commission->commission_value }}</td>
                                                <td>{{ $outlet_commission->party_title }}</td>
                                                <td>{{ $outlet_commission->commission_status }}</td>
                                                <td>{{ substr($outlet_commission->commission_description, 0, 40) }}...
                                                </td>
                                                <td>
                                                    {{ $outlet_commission->employee_name }}
                                                </td>
                                                <td>{{ $outlet_commission->created_at }}</td>
                                                <td>{{ $outlet_commission->updated_at }}</td>
                                                <td>
                                                    <form
                                                        action="{{ route('outlet-commissions.destroy', $outlet_commission->id) }}"
                                                        id="delete_item_from{{ $outlet_commission->id }}" method="post">
                                                        @csrf
                                                        @method('DELETE')
                                                        @if (!Auth::guard('web')->check())

                                                            {{-- @can('commission_show')
                                            <a class="btn p-0" title="View" href="{{ route('outlet-commissions.show',$outlet_commission->id)}}">
                                            ...
                                            @endcan --}}
                                                            ...
                                                            @can('commission_edit')
                                                                <a class="btn p-0" title="Edit"
                                                                    href="{{ route('outlet-commissions.edit', $outlet_commission->id) }}">
                                                            ...
                                                            @can('commission_delete')
                                                                <a class="btn p-0" title="Delete"
                                                                    onclick="deleteConfirmation('delete_item_from{{ $outlet_commission->id }}')"><i
                ...
                <form action="{{ route('categories.import') }}" enctype="multipart/form-data" method="post"
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            ...
                            <p>The correct column order is (category_title*, category_description) and you must follow this.
                            </p>
                        </div>
                        <div class="form-group row mb-0">
                            ...
                            <div class="col-xl-6">
                                <div class="form-group">
                                    ...
                                    <a href="{{ route('download.category-csv') }}"
